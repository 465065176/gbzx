    ...
    public class huayi
    {
        public static void FiddlerApplication_BeforeRequest(Session oSession) {
            if (
                ...
                (oSession.url.IndexOf("/zshy/cme/v1/CourseInfo/getCourseWareControl") >0) ||//https://api.91huayi.com/zshy/cme/v1/CourseInfo/getCourseWareControl
                (oSession.url.IndexOf("/zshy/cme/v1/CourseInfo/checkIfQuestion") > 0) ||//https://api.91huayi.com/zshy/cme/v1/CourseInfo/checkIfQuestion
                (oSession.url.IndexOf("/zshy/common/v1/exam/getExamQuestions") > 0) ||//https://api.91huayi.com/zshy/common/v1/exam/getExamQuestions
        ...
        public static void FiddlerApplication_BeforeResponse(Session oSession) {
            ...
            else if (oSession.url.IndexOf("/zshy/cme/v1/CourseInfo/getCourseWareControl") > 0)
            {
                oSession.utilDecodeResponse();
                var r = oSession.utilReplaceInResponse("\"if_exam_show\":0,", "\"if_exam_show\":1,");
                r = oSession.utilReplaceInResponse("\"if_face_course_ware_play\":1,", "\"if_face_course_ware_play\":0,");

            } else if (oSession.url.IndexOf("/zshy/cme/v1/CourseInfo/checkIfQuestion") > 0) {//
                oSession.utilDecodeResponse();
                var r = oSession.utilReplaceInResponse("\"reval\":\"2\",", "\"reval\":\"1\",");
            }
            else if (oSession.url.IndexOf("/zshy/common/v1/exam/getExamQuestions") > 0) {
                oSession.utilDecodeResponse();
                try
                {
                    ...
                    {
                        string answers = "";
                        for (int i = 0; i < dy.body.danxuan.Count; i++)
                        {
                            try
                            {
                                dynamic d = dy.body.danxuan[i];

                                Regex regex = new Regex("\\d+\\.(?<title>.*?)$");
                                string title = d.question_name.Value;
                                string result_id = d.result_id.Value;
                                for (int j = 0; j < d.question_options.Count; j++) {
                                    dynamic option = d.question_options[j];
                                    if (option.question_option_id.Value == result_id) {
                                        dy.body.danxuan[i].question_options[j].question_option_name.Value = dy.body.danxuan[i].question_options[j].question_option_name.Value + "====[对]";
                                    }
                                }
