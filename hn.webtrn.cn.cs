    ...
    public class webtrn
    {
        public static void FiddlerApplication_BeforeRequest(Session oSession) {
            if (
                ...
                (oSession.url.IndexOf("/u/student/study/myCourse.action") >0) ||//webtrn 课程列表
        ...
        public static void FiddlerApplication_BeforeResponse(Session oSession) {
            ...
            else if (oSession.url.IndexOf("/u/student/study/myCourse.action") > 0)
            {
                ...
                go = @"
                            try{
                                if($('.pull-left .active').text()!='未完成'){
                                    $('.pull-left a:contains(""未完成"")').click();
                                }else{
                                    var list=$('#courseList a:contains(""继续学习""),a:contains(""开始学习"")');
                                    if (list.length>0){
                                        list[0].click();
                                    }
                                }
